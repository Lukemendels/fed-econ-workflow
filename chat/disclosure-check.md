...
When two findings interact — e.g., a rough figure plus a specific quarter plus a small program name — flag the combination too; the aggregation risk in `controlled-information` applies here directly.

### Phase 4: Suggest remediation

For each CRITICAL or WARNING, propose the fix, in order of preference:

- **Replace** with a `<<FILL: description>>` placeholder (per the placeholder-discipline rules in `controlled-information` — describe the kind of value, don't smuggle the real one into the placeholder text).
...
- `controlled-information` — the standing protocol this skill pre-screens against: what never enters a prompt or repo, placeholder discipline, aggregation risk.
